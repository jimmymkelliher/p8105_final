...
merged_outcomes <- 
  merge(death_data_clean, hosp_data_clean)

merged_outcomes <- 
  merge(merged_outcomes, vacc_data_clean)
...
outcome_puma <- 
  merge(zcta_puma_clean, merged_outcomes) %>%
...
outcome_puma %>% pull(puma10) %>% unique
...
```{r}
a <- zip_to_zcta %>% mutate(zcta = as.numeric(zcta)) %>% pull(zcta) %>% unique()
