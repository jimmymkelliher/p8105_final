...
logistic_df = nyc_puma_summary %>% 
  mutate(
    below_vax_median = ifelse(covid_vax_rate > median(covid_vax_rate), 0, 1)
  ) %>% 
...
fit_logistic = 
  glm(below_vax_median ~ ., data = logistic_df, family = binomial())

# Algorithm does not converge, so we should use some form of penalized regression instead
x = model.matrix(below_vax_median ~ ., logistic_df)[,-1]
y = logistic_df$below_vax_median
