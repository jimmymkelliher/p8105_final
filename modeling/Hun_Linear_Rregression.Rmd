...
```{r}
nyc_hh_summary <- read_csv("./modeing/data_for_regression.csv")
...
```{r}
unbiased_data <- read_csv(".data/unibased_group_means.csv")
```
...
selected_variables_2 <-
  unbiased_data %>%
  select(health_insurance_private, health_insurance_public,
    race_asian_and_pacific_islander, education_bachelors_degree,
    on_foodstamps_yes, race_black, household_income, personal_income,
    on_welfare_yes, puma_vacc_per)
...
fit1 <- lm(puma_hosp_rate_2020  ~ language_spanish + education_bachelors_degree +
             ...
             birthplace_us:language_english, data = selected_variables)
