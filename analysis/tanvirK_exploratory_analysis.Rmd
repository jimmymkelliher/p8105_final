...
# Median Household Income and Vaccination Rate 
```{r}
income_vax_relation <-
  borough_info %>% 
...
# Median Household Income and Death Rate relation
```{r}
income_death_relation <-
  borough_info %>% 
  ggplot(aes(x = median_household_income,
             y = covid_death_rate)) + 
  geom_point() + 
  geom_smooth(
    ...
    se = FALSE) +
  labs(
    title = "Median Household-Income VS.COVID Death Rate",
    x = "Median household Income", 
    y = "Covid Death Rate"
  ) +
  scale_x_continuous(
...
# Foodstamp and Vaccination Rate Relationship 
```{r}
foodstmp_vaccination <- 
  borough_info %>% 
  ggplot(aes(x = perc_foodstamps,
             y = covid_vax_rate)) + 
  geom_point(aes(color = Borough)) + 
  geom_smooth(method = lm, formula = y ~ splines::bs(x, 3), se = FALSE) + 
  labs(
    title = "Foodstamp and Vaccination",
    x = "Foodstamp percentage in that puma for that Borough", 
    y = "Covid Vaccination Rate"
  ) +
  theme(legend.position = "right")
```
...
# Foodstamp and Death Rate Relationship 
```{r}
foodstmp_death <- 
  borough_info %>% 
  ggplot(aes(x = perc_foodstamps,
  ...
  labs(
    title = "Foodstamp and Death Rate",
    x = "Foodstamp percentage in that puma for that Borough", 
    y = "Covid Death Rate"
  ) +
  theme(legend.position = "none")
...
# Internet (braodband) and Death rate relationship 
```{r}
internet_death_rel <- 
  borough_info %>% 
  ggplot(aes(x = perc_broadband,
...
Median Personal Income and Vaccination Rate 
```{r}
borough_info %>% 
  ggplot(aes(x = median_personal_income,
             y = covid_vax_rate)) + 
  ...
  geom_smooth(
    ...
    se = FALSE) +
  labs(
    title = "Median Personal Income and Vaccination Rate", 
    subtitle = "By NYC Borough", 
    x = "Median Personal Income in that Puma for that Borough",
    y = "COVID Vaccination Rate") +
  theme(legend.position = "right")
