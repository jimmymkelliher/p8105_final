...
```{r}
# Maybe we can color each bar by borough split? Or we can reverse and color each bar by demographic, but factorize on X axis across boroughs?

# Let's try one example with race vs. hospitalization first
cleaned_data %>% 
  mutate(
    weighted_hosp = perwt * puma_hosp_rate
  ) %>% 
  ...
  summarize(
    race_hosp_rate = sum(weighted_hosp) / sum(perwt)
  ) %>% 
  ggplot(
    aes(
      x = reorder(race, race_hosp_rate),
      y = race_hosp_rate,
      fill = borough
    )
  ) + 
...
# Let's try another race vs. vaccination
cleaned_data %>% 
  mutate(
    weighted_vax = perwt * puma_vacc_rate
  ) %>% 
  group_by(race, borough) %>% 
  summarize(
    race_vax_rate = sum(weighted_vax) / sum(perwt)
  ) %>% 
  ggplot(
    aes(
      x = reorder(race, race_vax_rate),
      y = race_vax_rate,
      fill = borough
    )
  ) + 
...
# Here's an alternative but similar viz putting borough on the X and filling by race
cleaned_data %>% 
  mutate(
    weighted_hosp = perwt * puma_hosp_rate
  ) %>% 
  ...
  ggplot(
    aes(
      x = reorder(borough, race_hosp_rate),
      y = race_hosp_rate,
      fill = race
    )
  ) + 
...
# Number of people vaccinated by race, colored by borough
cleaned_data %>% 
  group_by(puma, race, borough) %>% 
  summarize(
    num_people = sum(perwt),
    puma_vacc_rate = median(puma_vacc_rate),
    number_vaxxed = num_people * puma_vacc_rate / 100,
    number_not_vaxxed = num_people - number_vaxxed
  ) %>% 
  group_by(race, borough) %>% 
  summarize(
    num_vaxxed = sum(number_vaxxed),
    num_not_vaxxed = sum(number_not_vaxxed)
  ) %>% 
  ggplot(aes(x = reorder(race, num_vaxxed), y = num_vaxxed, fill = borough)) + 
  geom_bar(stat = "identity")

# Percent vaxxed by borough, colored by race
cleaned_data %>% 
  group_by(puma, race, borough) %>% 
  summarize(
    num_people = sum(perwt),
  ...
  summarize(
    num_vaxxed = sum(number_vaxxed),
    num_not_vaxxed = sum(number_not_vaxxed),
    perc_vaxxed = num_vaxxed / (num_vaxxed + num_not_vaxxed)
  ) %>% 
  ggplot(aes(x = borough, y = perc_vaxxed)) + 
  geom_point(aes(color = race))

# Percent vaxxed by race, colored by borough
cleaned_data %>% 
  group_by(puma, race, borough) %>% 
  summarize(
    num_people = sum(perwt),
  ...
  ggplot(aes(x = race, y = perc_vaxxed)) + 
  geom_point(aes(color = borough))
```

```{r}
# Percent hospitalized by borough, colored by race
cleaned_data %>% 
  group_by(puma, race, borough) %>% 
  summarize(
    num_people = sum(perwt),
    ...
    number_not_hosp = num_people - number_hosp
  ) %>% 
  group_by(race, borough) %>% 
  summarize(
    num_hosp = sum(number_hosp),
    num_not_hosp = sum(number_not_hosp),
    perc_hosp = num_hosp / (num_hosp + num_not_hosp)
  ) %>% 
  ggplot(aes(x = borough, y = perc_hosp)) + 
  geom_point(aes(color = race))

# Percent dead by borough, colored by race
cleaned_data %>% 
  group_by(puma, race, borough) %>% 
  summarize(
    num_people = sum(perwt),
    puma_dead_rate = median(puma_death_rate),
    number_dead = num_people * puma_dead_rate / 100,
    number_not_dead = num_people - number_dead
  ) %>% 
  group_by(race, borough) %>% 
  summarize(
    num_dead = sum(number_dead),
    num_not_dead = sum(number_not_dead),
    perc_dead = num_dead / (num_dead + num_not_dead)
  ) %>% 
  ggplot(aes(x = borough, y = perc_dead)) + 
  geom_point(aes(color = race))
```

```{r}
# Manhattan seems to have the largest disparities by race on each outcome
# Is Manhattan also the most unequal on predictors like household income?
cleaned_data %>% 
  mutate(
...
```{r}
# Let's look at vaccination rate by income level
cleaned_data %>% 
  mutate(
...
# Let's look at vaccination rate by income level across PUMAs
cleaned_data %>% 
  mutate(
    income_class = cut(household_income, breaks = 10)
  ) %>% 
  group_by(income_class, puma) %>% 
  summarize(
    num_people = sum(perwt),
  ...
  summarize(
    people_vaxxed = sum(people_vaxxed),
    people_not_vaxxed = sum(people_not_vaxxed),
    percent_vaxxed = 100 * people_vaxxed / (people_vaxxed + people_not_vaxxed)
  ) %>% 
  ggplot(aes(x = income_class, y = percent_vaxxed)) + 
  geom_bar(stat = "identity")
...
cleaned_data %>% 
  mutate(
    income_class = cut(household_income, breaks = 5)
  ) %>% 
  group_by(income_class, puma, borough) %>% 
  summarize(
    num_people = sum(perwt),
    puma_vax_rate = median(puma_vacc_rate),
    people_vaxxed = num_people * puma_vax_rate / 100,
  ...
  group_by(income_class, borough) %>% 
  summarize(
    people_vaxxed = sum(people_vaxxed),
    people_not_vaxxed = sum(people_not_vaxxed),
    percent_vaxxed = 100 * people_vaxxed / (people_vaxxed + people_not_vaxxed)
  ) %>% 
  ggplot(aes(x = income_class, y = percent_vaxxed, color = borough, group = borough)) + 
  geom_point() + 
  geom_smooth()
...
cleaned_data %>% 
  ...
  ggplot(aes(x = perc_vaxxed, y = race, color = sex, fill = sex)) + 
...
cleaned_data %>% 
  ...
  ggplot(aes(x = perc_vaxxed, y = borough, color = race, fill = race)) + 
  geom_boxplot(alpha = 0.3)
